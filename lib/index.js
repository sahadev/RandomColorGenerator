...
exports.generatorColorTemplate = generatorColorTemplate;
exports.generatorColor = generatorColor;
...
const colorBaseEle = ['XX', 'FF', '00']; // 分治算法计算所有的颜色模板值
...
function generatorColorTemplate(array) {
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    for (let indexJ = 0; indexJ < temp.length; indexJ++) {
      ...
      resultArray.push("".concat(element).concat(element2));
    }
  }
...
colorTemplateArray = colorTemplateArray.concat(generatorColorTemplate(colorBaseEle)); // 获取颜色模板指针

const p = getColorTemplate(); // Generator Color

function generatorColor() {
  let increase = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  let templateStep = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 255;
  // 模板变换步长，默认255次返回变换一次模板。
  seek = Math.round(255 / templateStep); // 返回一个随机的颜色，例如:##FCBADE
  ...
  function* randomColor() {
    if (increase) {
      ...
      while (true) {
        const result = "#".concat(randomHex()).concat(randomHex()).concat(randomHex());
        console.info(result);
        yield result;
      }
    }
